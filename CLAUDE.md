...
**No real auth, anywhere.** `AdminProvider` (`components/AdminProvider.js`) stores a boolean `vestra_admin` flag in `localStorage`; `/admin` is both the entry screen (a single "Entrar como administrador" button, no password) and the dashboard home once the flag is set. `app/admin/layout.js` is the enforcement point — redirects to `/admin` if the flag isn't loaded yet, same UX-guidance-only pattern as `comanda-demo`'s `RoleProvider`. There's no server-side check re-validating admin status on the API routes; don't assume `/api/products` POST/PATCH/DELETE etc. are protected.
...
**Route groups.** Public storefront: `/` (landing + "quiénes somos"), `/tienda` (catalog, `?categoria=<slug>` filter), `/producto/[id]`, `/carrito` — all render `StoreHeader`. Admin: `/admin/*`, gated by `app/admin/layout.js` as described above; `/admin/productos` uses the shared `components/ProductForm.js` for both create and edit; `/admin/cupones` uses `components/CouponForm.js`. `app/api/<resource>/route.js` (+ `[id]/route.js`) are plain Route Handlers returning `{ error }` + a status code on failure, same as `comanda-demo`.
...
**Demo reset**: `POST /api/demo/reset` (`lib/demoReset.js`) wipes every table and reseeds the fixed catalog + 2 demo coupons, exposed as a button on `/admin` — same role as `comanda-demo`'s reset, since this is a public demo anyone can load garbage data into.
